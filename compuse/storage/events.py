...
class EventStore:
    def __init__(self, path=":memory:"):
        ...
        for pragma in ("PRAGMA foreign_keys=ON", "PRAGMA busy_timeout=10000", "PRAGMA synchronous=FULL"):
            self.db.execute(pragma)
        if self.path != ":memory:": self.db.execute("PRAGMA journal_mode=WAL")
        self.db.execute("CREATE TABLE IF NOT EXISTS events (run_id TEXT NOT NULL, seq INTEGER NOT NULL, type TEXT NOT NULL, payload TEXT NOT NULL, timestamp TEXT NOT NULL, previous_event_hash TEXT NOT NULL, event_hash TEXT NOT NULL, PRIMARY KEY(run_id,seq), UNIQUE(event_hash))")
        self.db.commit()
    ...
    @staticmethod
    def _hash(prev, run_id, seq, typ, body, timestamp):
        return hashlib.sha256(f"{prev}|{run_id}|{seq}|{typ}|{body}|{timestamp}".encode()).hexdigest()
    def append(self, run_id, event_type, payload, timestamp):
        ...
        try:
            with self.db:
                ...
                row = self.db.execute("SELECT seq,event_hash FROM events WHERE run_id=? ORDER BY seq DESC LIMIT 1", (run_id,)).fetchone()
                seq = row["seq"] + 1 if row else 1
                prev = row["event_hash"] if row else _ZERO_HASH
                h = self._hash(prev, run_id, seq, event_type, body, timestamp)
    ...
    def verify(self, run_id):
        prev = _ZERO_HASH
        try:
            rows = self.db.execute("SELECT * FROM events WHERE run_id=? ORDER BY seq", (run_id,))
            expected = 1
            for row in rows:
        ...
        return True
    def assert_integrity(self, run_id):
        if not self.verify(run_id): raise EventIntegrityError(f"event chain is invalid for run {run_id}")
